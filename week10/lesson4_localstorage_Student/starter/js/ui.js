...
const STORAGE_KEY = 'myQuotes'
function loadQuotes() {
  const storedData = localStorage.getItem(STORAGE_KEY)
  if (storedData) {
    try {
      const storedQuotes = JSON.parse(storedData)
...
function saveQuotes() {
  const currentQuotes = getAllQuotes()
  localStorage.setItem(STORAGE_KEY, JSON.stringify(currentQuotes))}
